...
function mouseClicked() {
    ...
    if (scene == 2) {
        if (mouseX > width/2-90 && mouseX < width/2+90 && mouseY > 57 && mouseY < 107) {
            scene = 1;
            explosion = undefined;
            obstacles = [];
            collison = false;
...
function drawEndScreen() {
  ...
  textAlign(CENTER, CENTER);
    ...
    for (i = 0; i < leaderboard.length; i++) {
        text(leaderboard[i][0], 370, 752 + 50*i);
        text(leaderboard[i][1], 533, 752 + 50*i);
    }
  pop();
  //column titles
  fill('#494740');
...
function keyPressed() {
    if (scene == 1 && explosion == undefined) {
